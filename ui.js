export function createTaskCard(task, {onEdit, onDelete}){
  ...
  if(task.color){
    if(card.style) card.style.backgroundColor = task.color; else card.setAttribute('data-color', task.color);
  } else if (task.quadrant === 1) card.classList.add('bg-red-50','border-l-red-500');
  ...
  if(card.style){
    ...
    card.setAttribute('data-x', task.x||0);
    card.setAttribute('data-y', task.y||0);
  }
